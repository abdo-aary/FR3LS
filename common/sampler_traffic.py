...
class TimeSeriesSampler(Dataset):
    ...
    def get_train_data_one_batch(self):
        Y = np.zeros(
            (len(self.train_batches_indices), self.train_window, self.timeseries.shape[1]))  # Y_B of shape (b, w, N)

        # Y_B[sampled_time_stamps_indices] = self.timeseries[sampled_index:sampled_index + self.train_window]
        for i in range(len(self.train_batches_indices)):
            sampled_index = self.train_batches_indices[i, 0]
            subseries_i = self.timeseries[sampled_index:sampled_index + self.train_window]
            Y[i] = subseries_i
