...
class ALDy(t.nn.Module):
    ...
    def forward(self, Y: t.Tensor):
        ...
        X_f_input = X[:1, self.f_input_indices, :].flatten(0, 1)  # Use first sample not the neg samples

        # R_means.shape = (ts2vec_output_dims,)  Summarizes the hole train_window's context variable, not f_input_window
        ...
        X_f_total = self.f_model(X_f_input, R_mean, self.f_out_same_in_size)

        # X_f_labels.shape = X_f_total.shape
        X_f_labels = X[:1, self.f_label_indices, :].flatten(0, 1)  # Use first sample not the neg samples
    ...
    def rolling_forecast(self, Y: t.Tensor, horizon: int):
        ...
        for j in range(X.shape[0]):  # Loop over the batch nbr
            for i in range(horizon):
                ...
                R_means = R[0].mean(dim=0)  # R_means of shape (ts2vec_output_dims,)

                # Forecast
                # X_f of shape (1, Cin_X) if f_out_same_in_size = False
                X_f = self.f_model(X[j:j + 1], R_means, self.f_out_same_in_size)
