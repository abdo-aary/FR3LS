...
class TLAE(t.nn.Module):
    ...
    def forward(self, Y: t.Tensor):
        ...
        X_f_input = t.cat(tuple(  # TODO: possible impr op: find a way how to optimize this cat operation
            X[:, i - self.f_input_window: i, :] for i in range(self.f_input_window, X.shape[1])
        ...
        if self.f_out_same_in_size:
            ...
            X_f_labels = t.cat(tuple(  # TODO: possible impr op: find a way how to optimize this cat operation
                X[:, i, :] for i in range(self.f_input_window, X.shape[1])
            ))  # X_f_labels.shape = (b * (train_window - f_input_window), f_input_window, latent_dim)
