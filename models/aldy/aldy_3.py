...
class ALDy(t.nn.Module):
    ...
    def forward(self, Y: t.Tensor, use_ts2vec: bool, paramGenTrain: bool = False):
        # Y of shape (N, Lin, Cin) Cin = nbr of channels, Lin = length of the signal sequence
        # Latent Series Generation
        if not use_ts2vec or paramGenTrain:
            X = self.encoder(Y[:1])
        else:
            X = self.encoder(Y)
    ...
    def rolling_forecast(self, Y: t.Tensor, horizon: int, use_ts2vec: bool = True):
        """
        ...
        return Y_forecast, X_inputs, out_weights
