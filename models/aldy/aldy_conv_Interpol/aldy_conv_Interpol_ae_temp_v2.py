...
class ALDy(nn.Module):
    def __init__(self, input_dim: int,
        ...
        self.out_channels = 8
        dConv = self.out_channels * 203

        self.conv_encoder1st = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=64, kernel_size=9, stride=9, padding=0),  ## out = N / 9 = 12787
        )
        self.conv_encoder = nn.Sequential(
            nn.ReLU(True),
            nn.Conv1d(in_channels=64, out_channels=20, kernel_size=7, stride=7, padding=1),  ## out = 1827
            nn.ReLU(True),
            nn.Conv1d(in_channels=20, out_channels=8, kernel_size=9, stride=9, padding=0),  ## out = 1827 / 9 = 203
            nn.ReLU(True),
            nn.Flatten()
        )
        ...
        self.conv_decoder = nn.Sequential(
            nn.ReLU(True),
            nn.ConvTranspose1d(in_channels=8, out_channels=20, kernel_size=9, stride=9, padding=0),
            nn.ReLU(True),
            nn.ConvTranspose1d(in_channels=20, out_channels=64, kernel_size=7, stride=7, padding=1),
            nn.ReLU(True),
            nn.ConvTranspose1d(in_channels=64, out_channels=1, kernel_size=9, stride=9, padding=0),
        )
    ...
    def forward(self, y, mask: str = None):
    ...
    def generate_apply_mask(self, y: t.Tensor, use_mask: bool = True, mask: str = None):
        # generate & apply mask
        x = t.clone(y)
