...
class TCN_Parameterized(nn.Module):
    ...
    def forward(self, x: t.Tensor, r: t.Tensor, f_out_same_in_size: bool):
        ...
        # self.tcn[-1].conv2.weight = t.nn.parameter.Parameter(self.tcn[-1].conv2.weight + weights)
        # self.tcn[-1].conv2.weight = t.nn.Parameter(self.tcn[-1].conv2.weight + ww)
        if weights.is_cuda and not self.tcn[-1].conv2.weight.is_cuda:
            device = weights.device
            self.tcn[-1].conv2.weight = self.tcn[-1].conv2.weight.to(device)
