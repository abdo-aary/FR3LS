...
def vae_trainer(snapshot_manager: SnapshotManager, model: t.nn.Module, training_set: Iterator,
                sampler: TimeSeriesSampler, horizon: int, val_loss_name: str, train_ae_loss: str,
                train_forecasting_loss: str, train_ts2vec_loss: str, lambda_ae: int = 1, lambda_f: int = 1,
                lambda_t2v: int = 1, epochs: int = 750, learning_rate: float = 0.001, verbose: bool = True,
                early_stopping: bool = False, patience: int = 5, ):
        ...
        for epoch_i in range(epoch + 1, epochs + 1):
            ...
            for _ in pb:
                ...
                training_batch_loss = lambda_ae * ae_loss_fn(prediction=y_hat, target=y, mu=mu,
                                                             sigma=sigma) + lambda_f * forecasting_loss_fn(
            ...
            y_forecast_normalized = model.rolling_forecast(y=input_windows_test_normalized, horizon=horizon)
            ...
            snapshot_manager.register(epoch=epoch_i, training_loss=training_loss, testing_loss=mape_l,
                                      other_losses_values=other_losses_values, save_model=optimized, model=model,
                                      optimizer=optimizer)
...
def vanilla_ae_trainer(snapshot_manager: SnapshotManager, model: t.nn.Module, training_set: Iterator,
                       sampler: TimeSeriesSampler, horizon: int, val_loss_name: str, train_ae_loss: str,
                       train_forecasting_loss: str, train_ts2vec_loss: str, lambda_ae: int = 1, lambda_f: int = 1,
                       lambda_t2v: int = 1, epochs: int = 750, learning_rate: float = 0.001, verbose: bool = True,
                       early_stopping: bool = False, patience: int = 5, ):
    ...
    with t.autograd.set_detect_anomaly(True):
        ...
        for epoch_i in range(epoch + 1, epochs + 1):
            ...
            snapshot_manager.register(epoch=epoch_i, training_loss=training_loss, testing_loss=test_loss,
                                      other_losses_values=other_losses_values, save_model=optimized, model=model,
                                      optimizer=optimizer)
...
def ae_t2v_trainer(snapshot_manager: SnapshotManager, model: t.nn.Module, training_set: Iterator,
                   sampler: TimeSeriesSampler, horizon: int, val_loss_name: str, train_ae_loss: str,
                   train_forecasting_loss: str, train_ts2vec_loss: str, lambda_ae: int = 1, lambda_f: int = 1,
                   lambda_t2v: int = 1, epochs: int = 750, learning_rate: float = 0.001, verbose: bool = True,
                   early_stopping: bool = False, patience: int = 5, ):
...
def raw_t2v_trainer(snapshot_manager: SnapshotManager, model: t.nn.Module, training_set: Iterator,
                    sampler: TimeSeriesSampler, horizon: int, train_ts2vec_loss: str, epochs: int = 750,
                    learning_rate: float = 0.001, verbose: bool = True, early_stopping: bool = False,
                    patience: int = 5, ):
    model = model.to(default_device())
    optimizer = t.optim.Adam(model.parameters(), lr=learning_rate)
    ts2vec_loss_fn = __loss_fn(train_ts2vec_loss)
    ...
    with t.autograd.set_detect_anomaly(True):
        ...
        for epoch_i in range(epoch + 1, epochs + 1):
            ...
            y_test = (labels_test - to_tensor(sampler.train_ts_means)) / to_tensor(sampler.train_ts_std)
            ...
            snapshot_manager.register(epoch=epoch_i, training_loss=training_loss, testing_loss=test_loss,
                                      save_model=optimized, model=model, optimizer=optimizer)
...
def ae_temp_trainer(snapshot_manager: SnapshotManager, model: t.nn.Module, avg_model: Union[AveragedModel, NoneType],
                    dataLoader: DataLoader, horizon: int, test_loss_name: str, train_ae_loss: str,
                    train_forecasting_loss: str, train_temp_loss: str, lambda_ae: float = 1, lambda_f: float = 1,
                    lambda_temp: float = 1, lambda_NC: float = 5e-3, epochs: int = 750, learning_rate: float = 0.001,
                    verbose: bool = True, pbar_percentage: int = 20, early_stopping: bool = False, patience: int = 5,
                    device_id: int = None, n_best_test_losses: int = None, lr_warmup: int = None, swa: bool = False, ):
    ...
    main_model = avg_model if swa else model
    epoch = snapshot_manager.restore(main_model, optimizer)

    if swa:
        model = avg_model.module
    ...
    with t.autograd.set_detect_anomaly(True):
        ...
        for epoch_i in range(epoch + 1, epochs + 1):
            ...
            for i, y in enumerate(dataLoader):
                ...
                y_hat, x_v1, x_v2, x_f_labels, x_f = model(y)
            ...
            with t.no_grad():
                ...
                # Forecast future values
                y_forecast_normalized = avg_model.module.rolling_forecast(y=input_windows_test_normalized,
                                                                   horizon=horizon) if swa else model.rolling_forecast(
                    y=input_windows_test_normalized, horizon=horizon)
                ...
                other_losses_values = {'MAPE': mape_test_loss, 'WAPE': wape_test_loss, 'SMAPE': smape_test_loss}
                snapshot_manager.register(epoch=epoch_i, training_loss=training_loss, testing_loss=test_loss,
                                          other_losses_values=other_losses_values, save_model=optimized,
                                          model=main_model, optimizer=optimizer)
...
def vae_tempC_trainer(snapshot_manager: SnapshotManager, model: t.nn.Module, training_set: Iterator,
                      sampler: TimeSeriesSampler, horizon: int, val_loss_name: str, train_ae_loss: str,
                      train_forecasting_loss: str, train_tempC_loss: str, train_distr_reg_loss: str, lambda_ae: int = 1,
                      lambda_f: int = 1, lambda_tempC: int = 1, lambda_distr: int = 1, epochs: int = 750,
                      learning_rate: float = 0.001, verbose: bool = True, early_stopping: bool = False,
                      patience: int = 5, ):
    model = model.to(default_device())
    optimizer = t.optim.Adam(model.parameters(), lr=learning_rate)
...
def t2vLR_trainer(snapshot_manager_t2v: SnapshotManager, snapshot_manager_LR: SnapshotManager, model: t.nn.Module,
                  avg_model: AveragedModel, sampler: TimeSeriesSampler, linear_regression: t.nn.Module,
                  dataLoader: DataLoader, horizon: int, ts2vec_loss_fn: str, train_LR_loss: str, test_LR_loss: str,
                  epochs_t2v: int, epochs_LR: int, learning_rate_t2v: float, learning_rate_LR: float,
                  verbose: bool = True, pbar_percentage: int = 20, early_stopping: bool = False, patience: int = 5,
                  device_id: int = None, n_best_test_losses: int = None, lr_warmup: int = None, temporal_unit: int = 0,
    ...
    with t.autograd.set_detect_anomaly(True):
        ...
        for epoch_i in range(epoch + 1, epochs_t2v + 1):
            ...
            with t.no_grad():
                # avg_model.eval()
                model.eval()
                _, labels_test, _ = map(to_tensor, sampler.test_forecasting_windows(), (device,) * 3)

                # Normalize labels windows
                y_test = (labels_test - to_tensor(sampler.train_ts_means,  # (n_test_windows, horizon, N)
                                                  device=device)) / to_tensor(sampler.train_ts_std, device=device)
                ...
                snapshot_manager_t2v.register(epoch=epoch_i, training_loss=training_loss, testing_loss=test_loss,
                                              save_model=optimized, # model=avg_model, optimizer=optimizer_t2v)
