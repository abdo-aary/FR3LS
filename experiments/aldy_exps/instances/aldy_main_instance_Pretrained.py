...
import numpy as np
from experiments.trainers.trainer import aldy_trainer_pretrained_no_warmup
from models.aldy.aldy_param_gen import ALDy
from models.aldy.aldy_param_gen_linear import ALDy as ALDy_Linear
...
class ALDyExperiment(Experiment):
    ...
    def instance(self,
        ...
        if f_model_type == 'TCN_Param_Linear':
            aldy_Model = ALDy_Linear
        else:
            aldy_Model = ALDy

        model = aldy_Model(input_dim=input_dim,
        ...
        if (f_model_path is not None) and (ts2vec_path is not None):
            ...
            # if f_model_type == 'TCN_Parameterized' or f_model_type == 'TCN_LinearParam':
            if 'TCN_Param' in f_model_type:
        ...
        trainer = aldy_trainer_pretrained_no_warmup
